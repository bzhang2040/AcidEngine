...
// The solar irradiance at the top of the atmosphere_
#define ATMOSPHERE_solar_irradiance vec3(1.474000, 1.850400, 1.911980)
// The sun's angular radius. Warning: the implementation uses approximations
// that are valid only if this angle is smaller than 0.1 radians.
#define ATMOSPHERE_sun_angular_radius float(0.004675)
// The distance between the planet center and the bottom of the atmosphere_
#define ATMOSPHERE_bottom_radius float(6360.000000)
// The distance between the planet center and the top of the atmosphere_
#define ATMOSPHERE_top_radius float(6480.000000 /* 6480.000000 */)
// The scattering coefficient of air molecules at the altitude where their
// density is maximum (usually the bottom of the atmosphere), as a function of
// wavelength. The scattering coefficient at altitude h is equal to
// 'rayleigh_scattering' times 'rayleigh_density' at this altitude.
#define ATMOSPHERE_rayleigh_scattering (vec3(0.005802, 0.013558, 0.033100))
// The scattering coefficient of aerosols at the altitude where their density
// is maximum (usually the bottom of the atmosphere), as a function of
// wavelength. The scattering coefficient at altitude h is equal to
// 'mie_scattering' times 'mie_density' at this altitude.
#define ATMOSPHERE_mie_scattering (vec3(0.003996, 0.003996, 0.003996))
// The extinction coefficient of aerosols at the altitude where their density
// is maximum (usually the bottom of the atmosphere), as a function of
// wavelength. The extinction coefficient at altitude h is equal to
// 'mie_extinction' times 'mie_density' at this altitude.
#define ATMOSPHERE_mie_extinction (vec3(0.004440, 0.004440, 0.004440))
// The asymetry parameter for the Cornette-Shanks phase function for the
// aerosols.
#define ATMOSPHERE_mie_phase_function_g float(0.800000)
// The extinction coefficient of molecules that absorb light (e.g. ozone) at
// the altitude where their density is maximum, as a function of wavelength.
// The extinction coefficient at altitude h is equal to
// 'absorption_extinction' times 'absorption_density' at this altitude.
#define ATMOSPHERE_absorption_extinction vec3(0.000650, 0.001881, 0.000085)
// The average albedo of the ground.
#define ATMOSPHERE_ground_albedo vec3(vec3(0.100000, 0.100000, 0.100000)
// The cosine of the maximum Sun zenith angle for which atmospheric scattering
// must be precomputed (for maximum precision, use the smallest Sun zenith
// angle yielding negligible sky light radiance values. For instance, for the
// Earth case, 102 degrees is a good choice - yielding mu_s_min = -0.2).
#define ATMOSPHERE_mu_s_min float(-0.207912*4.0)
...
#define kCamera vec3(0.0, 10.0 + cameraPosition.y/1000.0 + ATMOSPHERE_bottom_radius, 0.0)
...
#define atmosphereSampler skyLUT
...
bool RayIntersectsGround(float r, float mu) {
	//return false;
	return mu < 0.0 && r * r * (mu * mu - 1.0) + ATMOSPHERE_bottom_radius * ATMOSPHERE_bottom_radius >= 0.0;
}

float DistanceToTopAtmosphereBoundary(float r, float mu) {
	float discriminant = r * r * (mu * mu - 1.0) + ATMOSPHERE_top_radius * ATMOSPHERE_top_radius;
...
vec2 GetTransmittanceTextureUvFromRMu(float r, float mu) {
	...
	float H = sqrt(ATMOSPHERE_top_radius * ATMOSPHERE_top_radius - ATMOSPHERE_bottom_radius * ATMOSPHERE_bottom_radius);

	// Distance to the horizon.
	float rho = SafeSqrt(r * r - ATMOSPHERE_bottom_radius * ATMOSPHERE_bottom_radius);
...
vec3 GetTransmittanceToSun(float r, float mu_s) {
	...
	return GetTransmittanceToTopAtmosphereBoundary(r, mu_s) *
		smoothstep(-sin_theta_h * ATMOSPHERE_sun_angular_radius, sin_theta_h * ATMOSPHERE_sun_angular_radius, mu_s - cos_theta_h);
}
...
vec4 GetScatteringTextureUvwzFromRMuMuSNu(float r, float mu, float mu_s, float nu, bool ray_r_mu_intersects_ground) {
	...
	if (distance(r, ATMOSPHERE_bottom_radius) < 0.1) { r = ATMOSPHERE_bottom_radius; }
	...
	float r_mu = r * mu;
	float discriminant = r_mu * r_mu - r * r + ATMOSPHERE_bottom_radius * ATMOSPHERE_bottom_radius;
	...
	if (ray_r_mu_intersects_ground) {
		...
		float d_min2 = ATMOSPHERE_top_radius - r;
		float d_max2 = rho + H;
	...
	float ASDSA = -2.0 * ATMOSPHERE_mu_s_min * ATMOSPHERE_bottom_radius / (d_max - d_min);
...
vec3 PrecomputedSky(
	...
	float distance_to_top_atmosphere_boundary = -rmu - sqrt(rmu * rmu - r * r + ATMOSPHERE_top_radius * ATMOSPHERE_top_radius);
	...
	vec3 inScatter = (scattering * RayleighPhaseFunction(nu) + single_mie_scattering * MiePhaseFunction(ATMOSPHERE_mie_phase_function_g, nu)) * inTransmittance / 3.14;
...
vec3 PrecomputedSkyToPoint(
	...
	vec3 inScatter = (scattering * RayleighPhaseFunction(nu) + single_mie_scattering * MiePhaseFunction(ATMOSPHERE_mie_phase_function_g, nu)) * inTransmittance;
...
vec3 GetSolarRadiance() {
	return ATMOSPHERE_solar_irradiance / (PI * ATMOSPHERE_sun_angular_radius * ATMOSPHERE_sun_angular_radius) * SUN_SPECTRAL_RADIANCE_TO_LUMINANCE;
}
...
vec3 GetSunAndSkyIrradiance(vec3 point, vec3 normal, vec3 sun_direction, out vec3 sky_irradiance) {
	...
	// Direct irradiance.
	return ATMOSPHERE_solar_irradiance * GetTransmittanceToSun(r, mu_s);
}
